...
import {
  normalizeConsultationAnswersResult,
  useConsultationAnswers,
  useLatestForm,
...
const saveConsultationFormContext = (
  ...
  try {
    window.localStorage.setItem(
      getConsultationFormContextKey(consultationId, departmentId),
...
export default function ConsultationViewBackbone({
  ...
  const [departmentForm, setDepartmentForm] =
    useState<BackendDepartmentForm | null>(null);
  ...
  const hydrationStagesRef = useRef({
    answers: false,
    products: false,
    clinical: false,
  });
  ...
  const { loadConsultationAnswers: loadAnswers } = useConsultationAnswers(
    visitId,
    visitDepartmentId || null,
    departmentForm?.id || null,
  );
  const { loadVersionHistory } = useFormVersionHistory(
    departmentId || null,
    null,
  );
  const { upsertConsultationAnswers } = useUpsertConsultationAnswers();

  const isPendingDepartment =
    String(departmentStatus || "").toUpperCase() === "PENDING";
  ...
  const mapBackendFormVersion = (raw: any): BackendDepartmentForm => ({
    ...
    fields: Array.isArray(raw?.fields)
      ? raw.fields.map((field: any, idx: number) => normalizeField(field, idx))
      : [],
    sections: Array.isArray(raw?.sections)
      ? raw.sections.map((section: any, idx: number) =>
  ...
  const [actionListenerFieldId, setActionListenerFieldId] = useState<
    string | null
  >(null);
  const [fieldActions, setFieldActions] = useState<
    Record<string, FormAction[]>
  >({});
  const [showMissingProductsPrompt, setShowMissingProductsPrompt] =
    useState(false);
  const [missingProductsForField, setMissingProductsForField] = useState<
    Record<string, FormAction[]>
  >({});
  const [missingPromptHandled, setMissingPromptHandled] = useState(false);
  const [tableShapes, setTableShapes] = useState<
    Record<string, { rows: number; columns: number }>
  >({});
  const [showFinalizeConfirm, setShowFinalizeConfirm] = useState(false);
  const [diagnosticDrafts, setDiagnosticDrafts] = useState<
    Record<string, { diagnosis: string; description: string }>
  >({});
  ...
  const existingProductReferenceIds = Array.from(
    new Set([
      ...
      ...existingProducts.flatMap((product) =>
        extractProductIdentifiers(product),
      ),
    ]),
  );
  ...
  const [medicationMiniDrafts, setMedicationMiniDrafts] = useState<
    Record<string, { name: string; notes: string }>
  >({});
  ...
  const { removeProduct: removeVisitProduct } =
    useRemoveProductFromVisitDepartment();

  const [internalRequestProductsOpen, setInternalRequestProductsOpen] =
    useState(false);
  ...
  const { addProduct: addProductToVisitDepartment } =
    useAddProductToVisitDepartment();
  ...
  const requestProductsModalOpen =
    typeof requestProductsOpen === "boolean"
      ? requestProductsOpen
      : internalRequestProductsOpen;
  ...
  const selectedRequestDepartment = selectedRequestDepartmentId
    ? supportDepartments.find(
        (dept) => dept.id === selectedRequestDepartmentId,
      ) || null
    : null;
  ...
  const targetExistingChildVisitDepartment =
    targetExistingChildVisitDepartmentId
      ? childInvestigationDepartments.find(
          (child) =>
            String(child.id) === String(targetExistingChildVisitDepartmentId),
        ) || null
      : null;

  const isAppendingToExistingChild = Boolean(
  ...
  const showProductSearchComposer =
    ...
    showRequestComposer &&
    Boolean(selectedRequestDepartmentId && selectedRequestDepartment);
  ...
  const availableSupportDepartmentsForNewRequest = useMemo(
    () =>
      supportDepartments.filter(
        (dept) => !usedChildCatalogDepartmentIds.has(String(dept.id)),
      ),
  ...
  useEffect(() => {
    ...
    requestProductsModalOpen,
    childInvestigationDepartments.length,
    productsLocked,
  ]);
  ...
  const handleSubmitInvestigations = async () => {
    if (productsLocked) {
      setRequestErrorMessage(
        "Products cannot be changed on a completed visit or department.",
      );
      return;
    }
    if (!visitDepartmentId) {
      setRequestErrorMessage(
        "No active visit department to attach investigations to.",
      );
      return;
    }
    ...
    const invalidQuantity = pendingRequestProducts.find(
    ...
    try {
      const visitIdForRequest = String(
        initialConsultation.consultationId || visitId || "",
      );
      const existingChild =
        targetExistingChildVisitDepartment ||
      ...
      if (
        existingChild &&
        isEditableChildVisitDepartmentStatus(existingChild.status)
      ) {
        ...
        for (const item of pendingRequestProducts) {
          ...
          if (result?.status !== "SUCCESS") {
            const message =
              result?.messages?.[0]?.text || `Failed to add ${item.name}.`;
        ...
        const unitCount = pendingRequestProducts.reduce(
        ...
        if (result?.status !== "SUCCESS") {
          const message =
            result?.messages?.[0]?.text || "Failed to request investigations.";
          setRequestErrorMessage(message);
          return;
        }
    ...
    if (Array.isArray(value)) {
      return JSON.stringify(
        value
          ...
          .sort((a: any, b: any) =>
            `${a.id}:${a.name}:${a.diagnosis}`.localeCompare(
              `${b.id}:${b.name}:${b.diagnosis}`,
            ),
          ),
      );
    }
    ...
    const normalizedFieldActions = Object.keys(fieldActions)
      ...
      >((acc, fieldId) => {
        acc[fieldId] = (fieldActions[fieldId] || [])
          ...
          .sort((a, b) =>
            `${a.backendId}:${a.id}:${a.quantity}:${a.source}:${a.removedFromVisit}`.localeCompare(
              `${b.backendId}:${b.id}:${b.quantity}:${b.source}:${b.removedFromVisit}`,
            ),
          );
        return acc;
      }, {});
  ...
  const markHydrationStageComplete = (
    stage: "answers" | "products" | "clinical",
  ) => {
    ...
    const loadExistingAnswers = async () => {
      ...
      try {
        ...
        const {
          answer: answerData,
          form: responseForm,
          answersMap: parsedAnswersMap,
        } = normalizeConsultationAnswersResult(answersResult);
        const rawAnswers = parsedAnswersMap;
        const storedStatus = answerData?.status;
        const storedFormId = answerData?.formId
          ? String(answerData.formId)
          : "";
        const storedSchemaVersion = answerData?.formVersion
          ? String(answerData.formVersion)
          : "";

        setExistingSubmissionStatus(
          storedStatus === "FINAL"
            ? "FINAL"
            : storedStatus === "DRAFT"
              ? "DRAFT"
        ...
        if (
          responseForm &&
          (responseForm.fields?.length || responseForm.sections?.length)
        ) {
          ...
          if (
            String(departmentForm?.id || "") !==
            String(formForHydration.id || "")
          ) {
            setDepartmentForm(formForHydration);
          }
        } else if (storedFormId && storedSchemaVersion) {
          const currentVersion = String(
            departmentForm.currentSchemaVersion ||
              departmentForm.currentVersionNumber ||
              "",
          );
          const shouldResolveHistoricalVersion = Boolean(
            currentVersion && storedSchemaVersion !== currentVersion,
          );
          ...
          if (shouldResolveHistoricalVersion) {
            ...
            const versionCandidates =
              versionResult?.data?.getFormVersionHistory?.data || [];
            const exactVersion = versionCandidates.find(
              (candidate: any) =>
                String(candidate?.version || candidate?.versionNumber || "") ===
                storedSchemaVersion,
            );
        ...
        if (rawAnswers && typeof rawAnswers === "object") {
          saveConsultationFormContext(
            consultation.consultationId,
            String(visitDepartmentId || departmentId || ""),
            {
              consultationId: consultation.consultationId,
          ...
          if (existingProducts && existingProducts.length > 0) {
            ...
            existingProducts.forEach((liveProduct) => {
              ...
              const liveBackendId = String(
                liveProduct.backendId || liveProduct.id || "",
              );
                ...
                Object.keys(prev).forEach((fieldId) => {
                  next[fieldId] = prev[fieldId].map((action) => {
                    ...
                    const savedCatalogId = String(
                      action.rawData?.catalogItemId || action.rawData?.id || "",
                    );
                    if (!savedCatalogId) return action;

                    const liveBackendId =
                      liveBackendIdByCatalogId.get(savedCatalogId);
                    if (!liveBackendId) return action;
                    if (action.backendId === liveBackendId) return action;

                    console.log(
                      "[Consultation-Reconcile] Patching stale backendId for",
                      action.name,
                      {
  ...
  useEffect(() => {
    console.log(
      "[Consultation-Hydration] ===START=== existingProducts count:",
    ...
    if (!answersLoaded) {
      console.log(
        "[Consultation-Hydration] Early exit: answers not loaded yet",
      );
      return;
    }
    if (!existingProducts || existingProducts.length === 0) {
    ...
    const allFields = [
      ...
      ...(departmentForm.sections || []).flatMap(
        (section) => section.fields || [],
      ),
    ];
    const actionListenerFields = allFields.filter(
      (field) => field.type === "actionListener",
    );
    console.log(
      "[Consultation-Hydration] Found",
      actionListenerFields.length,
      "actionListener fields:",
      actionListenerFields.map((f) => f.id),
    );

    if (actionListenerFields.length === 0) {
      console.log(
        "[Consultation-Hydration] Early exit: no actionListener fields",
      );
      return;
    }
    ...
    actionListenerFields.forEach((field) => {
      ...
      const currentProductIds = new Set(
        currentFieldActions.flatMap((action) =>
          extractProductIdentifiers(action),
        ),
      );
      ...
      console.log("[Consultation-Hydration] Field:", field.id, {
        ...
        willShowPrompt:
          ...
          (currentFieldActions.length === 0 && existingProducts.length > 0),
      });

      if (missingProducts.length > 0) {
        console.log(
          "[Consultation-Hydration] SETTING PROMPT - missingProducts:",
        ...
        currentFieldActions.length === 0 &&
        existingProducts.length > 0
      ) {
        console.log(
          "[Consultation-Hydration] SETTING PROMPT - empty field with existing products",
        );
        ...
        console.log(
          "[Consultation-Hydration] No action needed for field:",
          field.id,
        );
      }
    });
    ...
    setFieldActions((prev) => {
      ...
      Object.keys(prev).forEach((fieldId) => {
        next[fieldId] = prev[fieldId].map((action) => {
          ...
          const existsInVisit = actionIds.some((id) =>
  ...
  useEffect(() => {
    if (!departmentForm || !answersLoaded || !visit?.departments?.length)
      return;

    const activeDepartment =
      visit.departments.find(
        (dep: any) => String(dep?.id) === String(visitDepartmentId || ""),
      ) || visit.departments[0];
    ...
    const diagnosticFields = allFields.filter(
      (field) => field.type === "diagnosticRecord",
    );
    const medicationLongFields = allFields.filter(
      (field) => field.type === "medicationLongForm",
    );
    const medicationMiniFields = allFields.filter(
      (field) => field.type === "medicationMiniForm",
    );

    const backendDiagnostics = Array.isArray(activeDepartment?.diagnostics)
      ? activeDepartment.diagnostics
      : [];
    const backendMedications = Array.isArray(activeDepartment?.medications)
    ...
    setFormAnswers((prev) => {
      ...
      diagnosticFields.forEach((field) => {
        ...
        const existingKeys = new Set(
          current.map(
            (item: any) =>
              `${String(item?.id || "")}:${String(item?.diagnosis || "").toLowerCase()}:${String(item?.description || "").toLowerCase()}`,
          ),
        );
        ...
        const missing = backendDiagnostics
          ...
          .filter(
            (item: { id: string; diagnosis: string; description?: string }) => {
      ...
      const targetMedicationFields = hydrateLongMedicationField
        ? medicationLongFields
        : medicationMiniFields;
      ...
      targetMedicationFields.forEach((field) => {
        ...
        const existingKeys = new Set(
          current.map(
            (item: any) =>
              `${String(item?.id || "")}:${String(item?.name || "").toLowerCase()}:${String(item?.notes || "").toLowerCase()}`,
          ),
        );

        const missing = backendMedications
          .map((item: any) => {
            const parsed = parseMedicationInstructions(
  ...
  useEffect(() => {
    ...
    if (!departmentForm || !answersLoaded || !consultation.consultationId)
      return;
    if (
      !hydrationStagesRef.current.answers ||
      !hydrationStagesRef.current.products ||
      !hydrationStagesRef.current.clinical
    )
      return;
  ...
  const createAnswersForProducts = async (
    fieldId: string,
    products: FormAction[],
  ) => {
    ...
    try {
      console.log(
        "[Consultation] Creating answers for field",
        fieldId,
        "with products:",
        products,
      );
      ...
      const normalized = buildAnswersForSubmission(
        formAnswers,
        tableShapes,
        tempFieldActions,
        departmentForm,
  ...
  const saveCurrentAnswersDraft = async (
    fieldActionsToSave: Record<string, FormAction[]> = fieldActions,
  ) => {
    ...
    try {
      ...
      if (saveResult?.status === "SUCCESS") {
        Object.keys(fieldActionsToSave).forEach((fieldId) => {
          lastSavedFieldActionsRef.current[fieldId] = fieldActionsToSave[
            fieldId
          ]
        ...
        allFields
          ...
          .forEach((field) => {
            lastSavedClinicalFieldsRef.current[field.id] =
              snapshotClinicalField(formAnswers[field.id]);
          });
  ...
  const handleBack = async () => {
    if (showMissingProductsPrompt && !missingPromptHandled) {
      console.log(
        "[Consultation] User is exiting while missing-products prompt is active; creating answers for those products before exit",
      );
  ...
  const handleAddAction = async (
    ...
    try {
      ...
      if (existingItem) {
        ...
        setFieldActions((prev) => ({
          ...prev,
          [actionListenerFieldId]:
            prev[actionListenerFieldId]?.map((a) =>
              a.id === existingItem.id ? { ...a, quantity: newQty } : a,
            ) || [],
        }));
        ...
        if (existingItem.backendId) {
          try {
            ...
            console.error(
              "Failed to update quantity when merging duplicate add:",
              err,
            );
          }
        }
      ...
      if (result?.status === "SUCCESS" && result.data) {
        ...
        if (
          ...
          const addedItem = result.data.products.find(
            (product: any) =>
              String(product?.product?.id) === String(searchResultId),
          );
          if (addedItem?.id) {
        ...
        if (!actualBackendId && Array.isArray(result.data?.departments)) {
          const dept = result.data.departments.find(
            (d: any) => String(d.id) === String(departmentIdParam),
          );
          if (Array.isArray(dept?.products) && dept.products.length > 0) {
            const addedItem = dept.products.find(
              (product: any) =>
                String(product?.product?.id) === String(searchResultId),
            );
            if (addedItem?.id) {
        ...
        if (!actualBackendId) {
          console.warn(
            `[handleAddAction] Failed to extract VisitDepartmentProduct ID for ${type} ${searchResultId}; using catalog ID as fallback`,
            { result },
          );
  ...
  const handleRemoveAction = async (fieldId: string, actionId: string) => {
    ...
    try {
      ...
      if (isSuccess || alreadyGone) {
        if (alreadyGone) {
          console.warn(
            "[ConsultationViewBackbone] Product already absent from visit; removing from field UI:",
            action.name,
          );
        }
        setFieldActions((prev) => ({
        ...
        console.error(
          "Failed to remove action/consumable via removeVisitDepartmentProduct:",
          result?.message,
        );
      }
    } catch (err) {
  ...
  const handleRestoreRemovedAction = async (
    fieldId: string,
    actionId: string,
  ) => {
    const action = fieldActions[fieldId]?.find((a) => a.id === actionId);
    ...
    try {
      ...
      if (
        ...
        const addedItem = result.data.products.find(
          (item: any) => String(item?.product?.id) === searchResultId,
        );
        if (addedItem?.id) {
          actualBackendId = String(addedItem.id);
        }
      ...
      if (!actualBackendId && Array.isArray(result?.data?.departments)) {
        ...
        const matchedDepartment = departments.find(
          (dep: any) => String(dep.id) === String(departmentIdArg),
        );
        if (
          Array.isArray(matchedDepartment?.products) &&
          matchedDepartment.products.length > 0
        ) {
          const addedItem = matchedDepartment.products.find(
            (item: any) => String(item?.product?.id) === searchResultId,
          );
          if (addedItem?.id) {
            actualBackendId = String(addedItem.id);
  ...
  const handleUpdateActionQuantity = async (
    fieldId: string,
    actionId: string,
    quantity: number,
  ) => {
    ...
    if (action?.backendId) {
      try {
        ...
        setFieldActions((prev) => ({
          ...
          [fieldId]:
            prev[fieldId]?.map((a) =>
              a.id === actionId ? { ...a, quantity: action.quantity } : a,
            ) || [],
        }));
      }
    }
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {showMissingProductsPrompt &&
        (() => {
          console.log(
            "[Consultation-Render] RENDERING PROMPT MODAL, missingProductsForField:",
            Object.keys(missingProductsForField),
          );
          return (
            <div className="fixed inset-0 z-50 flex items-center justify-center">
              ...
              <div className="bg-white rounded p-6 z-10 max-w-lg w-full shadow-lg">
                <h3 className="text-lg font-semibold mb-2">
                  Products found on visit
                </h3>
                <p className="mb-4">
                  We found products on the visit that are not in the current
                  form answers. Would you like to add them to the product
                  listener field?
                </p>
                <div className="mb-4">
                    ...
                    return (
                      <div key={fieldId} className="mb-3">
                        <div className="text-sm font-medium text-slate-700">
                          ...
                          <strong>{products.length}</strong> product(s)
                        </div>
                ...
                <div className="flex gap-2 justify-end">
                  ...
                  {existingSubmissionStatus === null ? (
                    <button
                      ...
                      onClick={async () => {
                        for (const fieldId of Object.keys(
                          missingProductsForField,
                        )) {
                          const prods = missingProductsForField[fieldId] || [];
                          if (prods.length > 0) {
                            const res = await createAnswersForProducts(
                              fieldId,
                              prods,
                            );
                            if (res?.status === "SUCCESS") {
                              setFieldActions((prev) => ({
                                ...prev,
                                [fieldId]: mergeUniqueByBackendId(
                                  prev[fieldId] || [],
                                  prods,
                                ),
                              }));
                              lastSavedFieldActionsRef.current[fieldId] = (
                                (prods || [])
                                  .map((p) => p.backendId)
                    ...
                    <button
                      ...
                      onClick={() => {
                        Object.keys(missingProductsForField).forEach(
                          (fieldId) => {
                            const prods =
                              missingProductsForField[fieldId] || [];
                            setFieldActions((prev) => ({
                              ...prev,
                              [fieldId]: mergeUniqueByBackendId(
                                prev[fieldId] || [],
      ...
      <Sheet
        open={requestProductsModalOpen}
        onOpenChange={setRequestProductsModalOpen}
      >
        <SheetContent
          side="right"
        ...
        >
          ...
          <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-4 py-4">
                    ...
                    {childInvestigationDepartments.map((childDept) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {isEditableChildVisitDepartmentStatus(
                            childDept.status,
                          ) &&
                            canAddMoreProducts && (
                              <Button
            ...
            {requestComposerMode === "other-service" &&
              canModifyVisitProducts && (
                  ...
                  {supportDepartmentsLoading ? (
                    ...
                    >
                      ...
                      <SelectContent className="z-[110]">
                        {availableSupportDepartmentsForNewRequest.map(
                          (dept) => (
                            <SelectItem
                              key={dept.id}
                              value={dept.id}
