...
export default function PatientHistorySidePane({
  ...
  const { departments: clinicDepartments, loading: clinicDepartmentsLoading } =
    useDepartments({ skip: !showFilters });
  ...
  const {
    ...
    error: historyError,
  } = useQuery(GET_PATIENT_HISTORY_QUERY, {
    variables: {
      patientId,
      input: historyInput,
    },
  ...
  const patient = useMemo(
    () => currentVisit?.patient || visits[0]?.patient || null,
    [currentVisit, visits],
  );

  const allDepartments = useMemo(() => {
    return clinicDepartments.map((dept) => ({
      id: String(dept.id),
      name: dept.name,
    }));
  }, [clinicDepartments]);
  ...
  const daysInMonth = useCallback(
    (year: number, month: number) => new Date(year, month, 0).getDate(),
    [],
  );
  ...
  const handleDepartmentClick = useCallback(
    (dept: VisitDepartment, visitId: string) => {
      if (!visitId) return;
                      ...
                      {filters.fromType === "day" && (
                        ...
                        >
                          {Array.from(
                            {
                              length: daysInMonth(
                                parseDateValue(filters.fromValue).year,
                                parseDateValue(filters.fromValue).month,
                              ),
                            },
                            (_, index) => index + 1,
                          ).map((day) => (
                      ...
                      {filters.toType === "day" && (
                        ...
                        >
                          {Array.from(
                            {
                              length: daysInMonth(
                                parseDateValue(filters.toValue).year,
                                parseDateValue(filters.toValue).month,
                              ),
                            },
                            (_, index) => index + 1,
                          ).map((day) => (
                ...
                <div className="space-y-1.5 mt-2">
                  {visit.departments?.map((dept) => {
                    const isCurrentDepartment =
                      visit.id === currentVisitId &&
                      dept.id === currentVisitDepartmentId;
                    const hasPreviewAnswer = Boolean(dept.answerId);
                    const disableDepartmentEntry =
                      isCurrentDepartment || !hasPreviewAnswer;
                    ...
                    return (
                      ...
                      >
                        <button
                          onClick={() =>
                            handleDepartmentClick(dept, visit.id || "")
                          }
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    Visit on{" "}
                    {formatDate(
                      visits.find((v) => v.id === expandedDepartment.visitId)
                        ?.visitDate,
                    )}
                  </p>
                </div>
