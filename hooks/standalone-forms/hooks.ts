...
export function useGetStandaloneForms(options?: {
  ...
  console.log('[useGetStandaloneForms] skip:', options?.skip, '| authToken:', token ? `${token.slice(0, 20)}...` : 'MISSING')

  const { data, loading, error, refetch } = useQuery(GET_STANDALONE_FORMS_QUERY, {
    variables: {
  ...
  const forms: StandaloneForm[] =
    data?.getStandaloneForms?.data ?? []

  return { forms, loading, error: error?.message ?? null, refetch }
}

/** Fetch a single standalone form by id */
export function useGetStandaloneForm(id: string | null, options?: { skip?: boolean }) {
  const token = typeof window !== 'undefined' ? localStorage.getItem('authToken') : null
  console.log('[useGetStandaloneForm] id:', id, '| skip:', !id || options?.skip, '| authToken:', token ? `${token.slice(0, 20)}...` : 'MISSING')

  const { data, loading, error, refetch } = useQuery(GET_STANDALONE_FORM_QUERY, {
    variables: { id },
    skip: !id || options?.skip,
    fetchPolicy: 'cache-and-network',
  })

  const form: StandaloneForm | null =
    data?.getStandaloneForm?.data ?? null
...
export function useCreateStandaloneForm() {
  const [mutate, { loading, error }] = useMutation(CREATE_STANDALONE_FORM_MUTATION)

  const createForm = async (input: StandaloneFormInput): Promise<StandaloneForm> => {
    const token = typeof window !== 'undefined' ? localStorage.getItem('authToken') : null
    console.log('[createStandaloneForm] input:', input, '| authToken:', token ? `${token.slice(0, 20)}...` : 'MISSING')
    const { data } = await mutate({ variables: { input } })
    if (data?.createStandaloneForm?.status === 'ERROR') {
      throw new Error(data.createStandaloneForm.message ?? 'Failed to create form')
    }
...
export function useUpdateStandaloneForm() {
  const [mutate, { loading, error }] = useMutation(UPDATE_STANDALONE_FORM_MUTATION)
  ...
  const updateForm = async (
    ...
    const token = typeof window !== 'undefined' ? localStorage.getItem('authToken') : null
    console.log('[updateStandaloneForm] id:', id, '| markFinal:', markFinal, '| authToken:', token ? `${token.slice(0, 20)}...` : 'MISSING')
    const { data } = await mutate({ variables: { id, input, markFinal } })
    if (data?.updateStandaloneForm?.status === 'ERROR') {
      throw new Error(data.updateStandaloneForm.message ?? 'Failed to update form')
    }
...
export function useDeleteStandaloneForm() {
  const [mutate, { loading, error }] = useMutation(DELETE_STANDALONE_FORM_MUTATION)

  const deleteForm = async (id: string, confirmDeleteAnswers = false): Promise<void> => {
    const token = typeof window !== 'undefined' ? localStorage.getItem('authToken') : null
    console.log('[deleteStandaloneForm] id:', id, '| authToken:', token ? `${token.slice(0, 20)}...` : 'MISSING')
    const { data } = await mutate({ variables: { id, confirmDeleteAnswers } })
    if (data?.deleteStandaloneForm?.status === 'ERROR') {
      throw new Error(data.deleteStandaloneForm.message ?? 'Failed to delete form')
    }
  }
...
export function useDuplicateStandaloneForm() {
  const [mutate, { loading, error }] = useMutation(DUPLICATE_STANDALONE_FORM_MUTATION)

  const duplicateForm = async (sourceFormId: string): Promise<StandaloneForm> => {
    const token = typeof window !== 'undefined' ? localStorage.getItem('authToken') : null
    console.log('[duplicateStandaloneForm] sourceFormId:', sourceFormId, '| authToken:', token ? `${token.slice(0, 20)}...` : 'MISSING')
    const { data } = await mutate({ variables: { sourceFormId } })
    if (data?.duplicateStandaloneForm?.status === 'ERROR') {
      throw new Error(data.duplicateStandaloneForm.message ?? 'Failed to duplicate form')
    }
...
export function useSaveStandaloneAnswer() {
  const [mutate, { loading, error }] = useMutation(SAVE_STANDALONE_ANSWER_MUTATION)
  ...
  const saveAnswer = async (
    ...
    const token = typeof window !== 'undefined' ? localStorage.getItem('authToken') : null
    console.log('[saveStandaloneAnswer] formVersionId:', formVersionId, '| authToken:', token ? `${token.slice(0, 20)}...` : 'MISSING')
    const { data } = await mutate({
      variables: { formVersionId, answers, status, score },
    })
    if (data?.saveStandaloneAnswer?.status === 'ERROR') {
      throw new Error(data.saveStandaloneAnswer.message ?? 'Failed to save answer')
    }
