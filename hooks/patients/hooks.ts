...
type RegisterPatientInsuranceInput = NonNullable<
  RegisterPatientInput["insurances"]
>[number];
...
export function useCreatePatientInsurance() {
  const [createPatientInsuranceMutation, { loading, error }] = useMutation(
    CREATE_PATIENT_INSURANCE_MUTATION,
  );

  const createPatientInsurance = async (
    input: PatientInsuranceMutationInput,
    ...
    try {
      const result = await createPatientInsuranceMutation({
        variables: {
          input: {
            ...
            principalMember:
              !input.dominantMember?.firstName?.trim() &&
              !input.dominantMember?.lastName?.trim() &&
              !input.dominantMember?.phone?.trim(),
            principalMemberName:
              [input.dominantMember?.firstName, input.dominantMember?.lastName]
                .filter(Boolean)
                .join(" ") || null,
            principalMemberPhoneNumber:
      ...
      return {
        ...
        messages: created?.message
          ? [{ text: created.message, type: created.status || "ERROR" }]
          : undefined,
        data: createdData
          ? mapInsuranceMutationResult(
              createdData,
              insuranceProvider,
              input.patientId,
            )
...
export function useUpdatePatientInsurance() {
  const [updatePatientInsuranceMutation, { loading, error }] = useMutation(
    UPDATE_PATIENT_INSURANCE_MUTATION,
  );
    ...
    try {
      ...
      return {
        ...
        messages: updated?.message
          ? [{ text: updated.message, type: updated.status || "ERROR" }]
          : undefined,
        data: updatedData
          ? mapInsuranceMutationResult(
              updatedData,
...
export function usePatients(
  ...
  const { data, loading, error, refetch } = useQuery<SearchPatientsQueryData>(
    GET_PATIENTS_QUERY,
    {
      variables: { input },
  ...
  const patients: Patient[] = (data?.searchPatients?.data || []).map(
    (gqlPatient: LocalGqlPatient) =>
...
export function usePatient(id: string | null) {
  const { data, loading, error, refetch } = useQuery<GetPatientQueryData>(
    GET_PATIENT_QUERY,
    {
  ...
  const patient = React.useMemo<Patient | undefined>(() => {
    ...
    mapped = {
      ...mapped,
      patientInsurances: patientInsurances.map((insurance) =>
...
export function useRegisterPatient() {
  const [registerPatientMutation, { loading, error }] = useMutation(
    REGISTER_PATIENT_MUTATION,
  );
          ...
          ? {
              ...
              patientInsurances:
                insuranceResults.length > 0
                  ? insuranceResults
                  : mapGqlPatient(updated.data).patientInsurances,
            }
          : undefined,
      };
    } catch (err) {
