...
import { getRecordIcon, getRecordColor } from "@/lib/mock-data"
...
export function AddRecordDialog({
  ...
  const [selectedTasks, setSelectedTasks] = useState<Map<string, ScheduledTask>>(new Map())
  ...
  useEffect(() => {
    if (!open) {
      ...
      // Set time to now when opening dialog
      setRecordTime(new Date())
    }
  }, [open])
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Check if any item is a discharge action
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="border rounded-lg p-3 space-y-3 bg-slate-50 dark:bg-slate-900">
                <div className="space-y-2">
                  ...
                  <Input
                    id="date"
                    type="date"
                    value={recordTime.toISOString().split("T")[0]}
                    onChange={(e) => {
                      const [year, month, day] = e.target.value.split("-").map(Number)
                      const newTime = new Date(recordTime)
                      newTime.setFullYear(year, month - 1, day)
                      if (admissionTime && newTime < admissionTime) {
                        return
                      }
                      setRecordTime(newTime)
                    }}
                    min={admissionTime ? admissionTime.toISOString().split("T")[0] : undefined}
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Input
                    id="time-input"
                    type="time"
                    value={recordTime.toISOString().slice(11, 16)}
                    onChange={(e) => {
                ...
                <div className="flex gap-2">
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    onClick={() => setRecordTime(new Date())}
                    className="flex-1"
                  >
                    Now
                  </Button>
                ...
                {admissionTime && recordTime < admissionTime && (
                  <p className="text-xs text-red-500">Time cannot be before admission ({format(admissionTime, "MMM dd, h:mm a")})</p>
                )}
              </div>
            )}
          </div>
          ...
          {items.length > 0 && (
            <Card className="p-4 bg-slate-50 dark:bg-slate-900">
              <Label className="text-sm font-semibold mb-3 block">Record Items ({items.length})</Label>
              <div className="space-y-2">
                {items.map((item) => (
                  <div key={item.id} className="flex items-center justify-between gap-2 p-2 bg-white dark:bg-slate-800 rounded border">
                    <div className="flex items-center gap-2">
                      <span className="text-xl">{getRecordIcon(item.type)}</span>
                      <span className="text-sm font-medium">{item.type.replace("-", " ")}</span>
                    </div>
                    <Button
                      type="button"
                      size="sm"
          ...
          <Card className="p-4 border-2 border-dashed">
            <Label className="text-sm font-semibold mb-3 block">Add Item to Record</Label>
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={items.length === 0 || !recordSummary.trim()}>
