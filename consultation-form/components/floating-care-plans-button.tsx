              ...
              <div className="space-y-2">
                {carePlans.length === 0 ? (
                  ...
                  carePlans.map((plan) => (
                    <div
                      key={plan.id}
                      className="flex items-start justify-between gap-2 p-2 rounded-lg bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700"
                    >
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium truncate">{plan.description}</p>
                        <p className="text-xs text-muted-foreground">
                          {plan.frequency} • {plan.scheduledTimes.join(", ")}
                        </p>
                      </div>
                      {currentUserRole === "Doctor" && (
