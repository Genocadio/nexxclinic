  ...
  plans.forEach((plan) => {
    plan.scheduledTimes.forEach((time) => {
      ...
      if (scheduledTime < now) {
        const completed = mockRecords.some((record) =>
          record.items.some((item) => item.type === plan.type) &&
...
export const getRecordColor = (type: RecordType, productType?: ProductType): string => {
  const colors: Record<RecordType, string> = {
    ...
    exam: "bg-green-100 text-green-700 dark:bg-green-950 dark:text-green-300",
    note: "bg-slate-100 text-slate-700 dark:bg-slate-950 dark:text-slate-300",
  }
  return colors[type]
}
