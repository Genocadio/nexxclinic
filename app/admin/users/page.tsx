...
export default function ManageUsersPage() {
  ...
  const filteredUsers = useMemo(() => {
    ...
    return users.filter((u) => {
      ...
      const departmentNames = (u.departments || []).map(
  ...
  const startEdit = (user: UserAccount) => {
    const userIsAdmin = (user.roles as string[] | undefined)?.includes(
      ADMIN_ROLE,
    );
    ...
    try {
      ...
      if (editingUserId) {
        ...
        if (
          ...
          toast.error(
            "You cannot edit your own roles here. Use My Account for profile and password updates.",
          );
          return;
        }
        ...
        if (
          editingUser &&
          (editingUser.roles as string[] | undefined)?.includes(ADMIN_ROLE) &&
          !canManageAdminUserAccounts
        ) {
        ...
        if (updateResp?.status !== "SUCCESS") {
          toast.error(
            updateResp?.messages?.[0]?.text || "Could not update user",
          );
          return;
        }
        ...
        if (createResp?.status !== "SUCCESS") {
          toast.error(
            createResp?.messages?.[0]?.text || "Could not create user",
          );
          return;
        }
  ...
  const handleToggleActive = async (user: UserAccount) => {
    ...
    try {
      ...
      if (response?.status !== "SUCCESS") {
        toast.error(
          response?.messages?.[0]?.text || "Could not update activation",
        );
        return;
      }
  ...
  const handleRequirePasswordSetup = async (user: UserAccount) => {
    ...
    try {
      ...
      if (response?.status !== "SUCCESS") {
        toast.error(
          response?.messages?.[0]?.text || "Could not reset password state",
        );
        return;
      }
              ...
              <DialogHeader>
                ...
                <DialogDescription>
                  {editingUserId
                    ...
                    : "Fill in the details below to create and register a new system user."}
                </DialogDescription>
              </DialogHeader>
        ...
        >
          <DialogContent className="sm:max-w-xl backdrop-blur-xl bg-white/10 dark:bg-black/25 border border-white/20 rounded-3xl shadow-2xl p-3">
            <div className="bg-[#FBF2ED] dark:bg-slate-900 border border-border/40 dark:border-slate-800 rounded-2xl p-6 shadow-lg">
              ...
              <form
                onSubmit={handleConfirmActivation}
            ...
            <div className="space-y-2">
              {filteredUsers.map((user) => {
                ...
                const canManageThisUser =
                  !selfUser && (canManageAdminUserAccounts || !userIsAdmin);
                  ...
                  >
                    <div>
                      <div className="flex items-center flex-wrap gap-2">
                        ...
                        <span
                          className={`px-2 h-6 rounded-full border text-xs inline-flex items-center ${userIsAdmin ? "border-primary/30 bg-primary/10 text-primary" : "border-border text-muted-foreground"}`}
                        >
                          {userIsAdmin ? "Admin" : "Non-admin"}
                        </span>
                        <span
                          className={`px-2 h-6 rounded-full border text-xs inline-flex items-center ${
                      ...
                      <div className="flex flex-wrap gap-1 mt-2">
                        {(user.roles || []).map((role) => (
                          <span
                            key={`${user.id}-${role}`}
                            className="px-2 h-6 rounded-full border border-border text-xs inline-flex items-center"
                          >
                            {role}
                          </span>
                        ))}
